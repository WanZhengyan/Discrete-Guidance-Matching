...
class MixtureDiscreteEulerSolver_RateGuidance(Solver):
    ...
    def __init__(
        ...
        guidance_model: Optional[ModelWrapper] = None
    ):
        super().__init__()
        self.model = model
        self.path = path
        ...
        with ctx:
            for i in range(n_steps):
                ...
                if i == n_steps - 1:
                    ...
                    if self.guidance_model is not None and guidance_strength > 0:
                        guidance_factor = torch.zeros(x_t.shape, device=x_t.device) # shape: (batch, length)
                        for d in range(x_t.shape[1]):
                            z = x_t.clone()
                            z[:,d] = x_1[:,d] # shape: (batch, length)
                            guidance_factor[:,d] = self.guidance_model(x=z, t=t.repeat(x_t.shape[0])).squeeze(-1) 
                        guidance_factor = ((guidance_factor + 1e-16) / (self.guidance_model(x=x_t, t=t.repeat(x_t.shape[0])) + 1e-16)) ** guidance_strength
                        u = u * guidance_factor.unsqueeze(-1)
